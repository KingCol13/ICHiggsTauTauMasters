...
met_cov = [remove_nan(df4["metcov00"]), remove_nan(df4["metcov01"]),remove_nan(df4["metcov10"]), remove_nan(df4["metcov11"]) ]

################################################## A ###############################################################

third_best_loss = 20000
third_best_ID = 0
second_best_loss = 20000
second_best_ID = 0
best_loss = 20000
best_ID = 0
...
def test_scenario(scenario_nb, list_inputs, trainFrac = 0.7, nodes1 = 300, nodes2 = 300, drop = 0.1, E_size = 25, B_size = 50, ratio_all = 1/10000, ratio_p = 10/1500, ratio_phi = 4, ratio_tau = 10, ratio_H = 4, NN_structure = 'z'):
    ...
    global third_best_loss, third_best_ID, second_best_loss, second_best_ID, best_loss, best_ID, third_best_NN_struc, second_best_NN_struc, best_NN_struc
    
    print(second_best_NN_struc)
...
#now define the NN structure, depends on the input
    ...
    if loss_val <= best_loss :
        ...
        third_best_ID = second_best_ID 
        third_best_NN_struc = second_best_NN_struc
        
        second_best_loss = best_loss
        second_best_ID = best_ID
        second_best_NN_struc = best_NN_struc
        
        best_loss = loss_val
        best_ID = scenario_nb
        best_NN_struc = NN_structure
        
    if loss_val <= second_best_loss and loss_val != best_loss:
        third_best_loss = second_best_loss
        third_best_ID = second_best_ID 
        third_best_NN_struc = second_best_NN_struc
        
        second_best_loss = loss_val
        second_best_ID = scenario_nb
        second_best_NN_struc = NN_structure
    
    if loss_val <= third_best_loss and loss_val != best_loss and loss_val != second_best_loss:
        third_best_loss = loss_val
        third_best_ID = scenario_nb
        third_best_NN_struc = NN_structure

    #raise End

    table = np.array(table).T.tolist()
    table.append([0., 0., 0., 0.])
    table.append([ratio_all, ratio_p, ratio_phi, ratio_tau])
    table.append([ratio_H, 0., 0., 0.])
    table.append([E_size, B_size, trainFrac, 0.])
    table.append([nodes1, nodes2, drop, 0.])
    table.append([loss_val, 0., 0., 0.])

    np.savetxt('All%i_a1_a1_%i_%i_%i_%i_%i_%s.csv'%(scenario_nb, ratio_all, ratio_p, ratio_phi, ratio_tau, ratio_H, NN_structure), table, delimiter=',', fmt = '%.2e')
    return 1
...
nu1_guess, nu2_guess = polari.polarimetric(df4, 10, 10)
...
alpha_1, alpha_2 = am.alphas_clamped(df4, 10, 10)
...
x11 = np.array([
              ...
              Mom4_to_tf(tau_2_vis.p_y),  #5
              Mom4_to_tf(tau_2_vis.p_z),  #5
              remove_nan(df4["ip_x_1"]), remove_nan(df4["ip_y_1"]), remove_nan(df4["ip_z_1"]), #6
              remove_nan(df4["ip_x_2"]), remove_nan(df4["ip_y_2"]), remove_nan(df4["ip_z_2"]), #7
              #df4["ip_sig_2"], df4["ip_sig_1"], #8,9
              remove_nan(df4["met"]),                #1
              remove_nan(df4["metx"]),remove_nan(df4["mety"]),   #2,3
              #df4["sv_x_1"], df4["sv_y_1"], df4["sv_z_1"], #10
              #df4["sv_x_2"], df4["sv_y_2"], df4["sv_z_2"], #11
              #*sv_cov,
              #*ip_cov,
...
third_best_loss = 20000
third_best_ID = 0
third_best_NN_struc = 'n'
second_best_loss = 20000
second_best_ID = 0
second_best_NN_struc = 'n'
best_loss = 20000
best_ID = 0
best_NN_struc = 'n'


for i in [3, 5, 6, 7, 8, 11, 17, 19, 24]:  
    for j in ['z', 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h']:
        exec('done = test_scenario(%s, x%s, trainFrac_in, nodes1_in, nodes2_in, drop_in, E_size_in, B_size_in, ratio_all, ratio_p, ratio_phi, ratio_tau, ratio_H, "%s" )'%(i,i,j))
        
        results = ['Configuration loss:%i, %i, %i, %i, %i:\nBest loss: %.2f, Best configuration: %s,%i \nSecond best loss: %.2f, second best configuration: %s, %i\nThird best loss: %.2f, Third best configuration: %s, %i' % (ratio_all, ratio_p, ratio_phi, ratio_tau, ratio_H, best_loss, best_NN_struc, best_ID, second_best_loss, second_best_NN_struc, second_best_ID, third_best_loss, third_best_NN_struc, third_best_ID)]

        np.savetxt('Results_NN_a1_a1_%i_%i_%i_%i_%i.txt'%(ratio_all, ratio_p, ratio_phi, ratio_tau, ratio_H), results, delimiter=',', fmt = '%s')
...
#in order to save the three best configurations
third_best_loss = 20000
third_best_ID = 0
third_best_NN_struc = 'n'
second_best_loss = 20000
