...
#working in the a1(3pi)-pi channel
tau_mode1 = 10
tau_mode2 = 10
decay_mode1 = 10
decay_mode2 = 0
...
nu_1_guess1, nu_1_guess2, nu_2_guess1, nu_2_guess2, best_guess_nu1, best_guess_nu2, df4['nb_tau_shift'] = polari.polarimetric_full(df4, decay_mode1, decay_mode2, 'sv', 'tau_p', True)


print(df4['nb_tau_shift'])

#'only taking the re-shifted taus, i.e. one solution
#df4 = df4[(df4['nb_tau_shift'] == 1)]
...
gen_tau_2 = tau_2_vis + nu_2

#in visible decay products frame
boost =  tau_1 +  gen_tau_2 
tau_1_vis = tau_1_vis.boost_particle(boost)
tau_1 = tau_1.boost_particle(boost)
tau_2_vis = tau_2_vis.boost_particle(boost)
gen_tau_2 = gen_tau_2.boost_particle(boost)


#1 set up the two planes
#IP_vect_2 = [df4['ip_x_2'], df4['ip_y_2'], df4['ip_z_2']]

IP_vect_2 = [gen_tau_2.p_x, gen_tau_2.p_y, gen_tau_2.p_z]
IP_vect_2 = np.transpose(IP_vect_2)/bf.norm(np.transpose(IP_vect_2))
IP_vect_2 = np.transpose(IP_vect_2)

pi_vect_2 = [tau_2_vis.p_x, tau_2_vis.p_y, tau_2_vis.p_z]
pi_vect_2 = np.transpose(pi_vect_2)/bf.norm(np.transpose(pi_vect_2))
pi_vect_2 = np.transpose(pi_vect_2)

#vector director of plane spanned by IP and pion     
n_pion = bf.cross_product(IP_vect_2, pi_vect_2)

tau_vect_1 = [tau_1.p_x, tau_1.p_y, tau_1.p_z]
tau_vect_1 = np.transpose(np.transpose(tau_vect_1)/bf.norm(np.transpose(tau_vect_1)))

a1_vect_1 = [tau_1_vis.p_x, tau_1_vis.p_y, tau_1_vis.p_z]
a1_vect_1 = np.transpose(np.transpose(a1_vect_1)/bf.norm(np.transpose(a1_vect_1)))

n_a1 = bf.cross_product(tau_vect_1, a1_vect_1)

df4['pv_angle_IP'] = np.arccos(np.clip(bf.dot_product(n_pion, n_a1),-1, 1))
plt.hist(df4['pv_angle_IP'], bins = 100)
plt.hist(df4['aco_angle_5'], bins = 100)
plt.show()
...
plt.title('checks a1-pi channel')
plt.hist(df_sm['pv_angle_IP'], alpha = 0.5, bins = 50, label = 'sm pv_angle_IP')
plt.hist(df_ps['pv_angle_IP'], alpha = 0.5, bins = 50, label = 'ps pv_angle_IP')
...
ax.scatter(df4['ip_x_2'][0], df4['ip_y_2'][0], df4['ip_z_2'][0])
ax.scatter(0,0,0)
ax.scatter(pi_vect_2[0][0], pi_vect_2[0][1], pi_vect_2 [0][2])
ax.scatter(0,0,0)

ax.scatter(a1_vect_1[0][0], a1_vect_1[0][1], a1_vect_1[0][2])
plt.show()
