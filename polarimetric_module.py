...
def polarimetric(df, decay_mode1, decay_mode2):
    ...
    theta_GJ = np.arccos(np.clip(bf.dot_product(tau1_dir, vis_1_dir/bf.norm(vis_1_dir)), -1, 1))
    theta_GJ_max = np.arcsin((m_tau**2 - m_vis**2)/(2*m_tau*tau_1_vis.p))
    ...
    cos_theta_GJ =np.clip(bf.dot_product(tau2_dir, vis_2_dir/bf.norm(vis_2_dir)), -1, 1)
    theta_GJ = np.arccos(cos_theta_GJ)
    theta_GJ_max = np.arcsin((m_tau**2 - m_vis**2)/(2*m_tau*tau_2_vis.p))
    theta_GJ = tf.where(theta_GJ >= theta_GJ_max, theta_GJ_max, theta_GJ)
    ...
    return best_guess_nu1, best_guess_nu2
